...
import main.java.serializers.ResponseSerializerImpl;
        ...
        if (httpRequest.getMethod().equals("GET")) {
            ...
            if (!Files.exists(path)) {
                ...
                httpResponse = HttpResponse.createBuilder()
                        .withStatusCode("HTTP/1.1 404 NOT FOUND")
                        .withBody("Content-Type: text/html; charset=utf-8")
                        .withHeader("<h1>File not found</h1>")
                                .build();
//                response.append("HTTP/1.1 404 NOT FOUND\n");
//                response.append("Content-Type: text/html; charset=utf-8\n");
//                response.append("\n");
                socketService.writeResponse(responseSerializer.serialize(httpResponse),
                        new StringReader(httpResponse.getHeader()));
//                        socketService.writeResponse(responseSerializer.serialize(httpResponse),
//                        new StringReader("<h1>File not found</h1>\n"));
            ...
            //TODO use implementation of ResponseSerializer
            httpResponse = HttpResponse.createBuilder()
                    .withStatusCode("HTTP/1.1 200 OK")
                    .withBody("Content-Type: text/html; charset=utf-8").build();
...
//                socketService.writeResponse(response.toString(), Files.newBufferedReader(path));
            ...
            //TODO use implementation of ResponseSerializer
            httpResponse = HttpResponse.createBuilder()
                    .withStatusCode("HTTP/1.1 405 METHOD_NOT_ALLOWED")
                    .withBody("Content-Type: text/html; charset=utf-8")
                    .withHeader("<h1>Method not allowed</h1>")
                    .build();
